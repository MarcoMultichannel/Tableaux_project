...
public class Tableaux implements OWLReasoner {
    ...
    List<OWLAxiom> getUnfoldableComponent(List<OWLAxiom> axioms) {
        ...
        for (OWLAxiom axiom : axioms) {
            if (parser.isSubClassOfAxiom(axiom) && !checkIfGraphAcyclic(axiom)) {
                // if true then add axiom to unfoldableGraph for future checks and add axiom to result
    ...
    public void createAndAddVertex(OWLAxiom axiomTemp){
        // in this method we add axiom to unfoldableGrapfTbox, for future checks
          try {
    ...
    public boolean checkEquivalenceMethod(OWLAxiom axiomTemp,List<OWLAxiom> unfoldableAxioms){
        // here we check if left part of an equivalence is the left part of an inclusion or equivalnce in Tu (unfoldable part)
        boolean res = false;
        try {
          ...
          // now check if sx is on the left of an inclusion or equivalence in result (here is axioms)
    ...
    public boolean checkIfGraphAcyclic(OWLAxiom axiomTemp) {
        // in this method we check if adding axiomTemp in a graph similar to unfoldableGrapfTbox we have a cycle
        boolean res = false;
        try {
            // first create a graph temp equals to unfoldableGraphTBox;
            GraphUnfoldable unfoldableGraphTemp =  new GraphUnfoldable(unfoldableGraphTBox);
            // now we take left and right part of axiom
            List<OWLClassExpression> ce = parser.unpackSubClassAxioms((OWLSubClassOfAxiom) axiomTemp);
            OWLClassExpression subclass = ce.get(0);
            OWLClassExpression superclass = ce.get(1);
            //now adding Vertex and edges
